...
//
// Draws the Eaon Agent icon: a full-bleed rounded square in the agent's own
// amber-workshop palette — vertical warm-black gradient, soft radial highlight
// top-left, a bold amber chevron prompt glyph, and a thin amber baseline.
...
// Palette — the Eaon theme: warm near-black surface, amber ink.
const TOP = [0x2b, 0x20, 0x0d];   // #2b200d
const BOT = [0x11, 0x0d, 0x07];   // #110d07
const GLYPH = [0xf4, 0xb9, 0x42]; // prompt chevron, Eaon amber
const ACCENT = [0xf8, 0xd7, 0x9a]; // underline, lighter amber
const HILITE = [0xff, 0xe3, 0xb2]; // radial highlight tint
...
const GX0 = 320;       // left edge of both bars
const GX1 = 672;       // tip x
const GY  = 512 - 168; // upper bar centerline y
const GT  = 74;        // bar thickness
const UNDERLINE = { x: 512, y: 762, hw: 104, hh: 8 }; // accent bar
...
function render() {
  ...
  for (let y = 0; y < RENDER; y++) {
    ...
    for (let x = 0; x < RENDER; x++) {
      ...
      // Coverage is 0..1; the buffer is in 8-bit channel space like the color
      // channels, so scale it or the whole icon comes out transparent.
      ...
      // The SDF is already offset by half the stroke width, so coverage is
      // just the distance to the outline — half a pixel of antialiasing.
      const cs = chevronSDF(x, y, 2);
      const cg = clamp(0.5 - cs, 0, 1);
      if (cg > 0) {
