...
// chrome stays fixed — same as PgUp/PgDn but trackpad/mouse driven.
stdin.write("\x1b[<64;50;10M"); // wheel up anywhere over the chat
await waitFor("wheel up scrolls the chat", () => (uiFrame() ?? "").includes("scrolled —"));
check("chrome fixed while wheel-scrolled", (uiFrame() ?? "").includes("EAON") && (uiFrame() ?? "").includes("Ready"));
stdin.write("\x1b[<65;50;10M"); // wheel down
stdin.write("\x1b[<65;50;10M");
stdin.write("\x1b[<65;50;10M");
await waitFor("wheel down returns to live view", () => !(uiFrame() ?? "").includes("scrolled —"));
check("frame still fits after wheel scrolling", (uiFrame() ?? "").split("\n").length <= ROWS);
