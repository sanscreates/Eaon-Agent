...
```
/help /model /models /providers /compress /clear /stats /theme /init /setup /exit
/macro list|set|rm        manage output macros
/permissions confirm|auto|readonly
```

`/providers` opens an interactive provider manager (edit display name, base URL or API key; add, remove or rename models; delete providers). You can also script it: `/providers list`, `/providers rm <id>`, `/providers rename <id> <name>`, `/providers url <id> <url|-`, `/providers key <id> <key|->`, and `/providers add-model/rm-model/rename-model <id> <model>`. The same editing and deleting works in the Mac app under Settings → Models.
...
- **scrollable chat history**: the chrome (top bar, session header, input, status bar) never moves; only the conversation scrolls. Use the **mouse wheel or trackpad**, `PgUp`/`PgDn`, or `Ctrl+U`/`Ctrl+D` on keyboards without paging keys to move through history and return to the live view
- animated first-load splash: gradient ASCII logo, a typing quote, and key buttons — `Enter` to start, `S` to open setup, `Q` to quit
