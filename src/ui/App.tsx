...
export function App(props: { rt: Runtime; forceSetup?: boolean }): React.ReactElement {
  ...
  // Mouse reporting for the whole session: the wheel/trackpad scrolls the
  // chat viewport. Clicks are reported too but deliberately ignored.
  ...
  useInput((input, key) => {
    ...
    // Mouse reports: the wheel/trackpad scrolls the chat like PgUp/PgDn.
    // Everything else (clicks, releases) is swallowed here so it can never
    // reach a text input as literal junk.
    if (isMouseInput(input)) {
      const delta = wheelDelta(input);
      if (delta !== 0 && overlay === "none" && !needsOnboarding) scrollBy(delta * 3);
      return;
    }
    if (overlay === "welcome") {
  ...
  return (
    <Box flexDirection="column" width={terminalSize.columns} height={terminalSize.rows} paddingX={1}>
      ...
      {overlay === "welcome" && !needsOnboarding ? (
        <WelcomeScreen theme={theme} workspace={workspace} mainLabel={mainLabel} terminalRows={terminalSize.rows} columns={terminalSize.columns} />
      ) : (
            ...
            {inSetup ? (
              ...
              <>
                ...
                <Box flexDirection="column" flexGrow={1} overflow="hidden" justifyContent="flex-end" paddingX={1}>
                  ...
                  {effectiveOffset > 0 ? (
                    <Text dimColor>  ↓ scrolled — {effectiveOffset} line{effectiveOffset === 1 ? "" : "s"} below · PgDn / ^D / wheel returns to live view</Text>
                  ) : null}
