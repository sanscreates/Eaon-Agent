...
const WELCOME_BUTTONS = ["⏎ start", "s setup", "q quit"];
const BUTTON_GAP = 2;

/**
 * First-load splash: gradient ASCII logo revealed line by line, a welcome
 * card with key buttons, and a quote that types itself out. The layout fits
 * exactly under the top bar: on short terminals the ASCII art drops out so
 * nothing ever overflows the fixed root.
 */
export function WelcomeScreen(props: {
  ...
  // ---- deterministic fit: the splash must sit exactly under the top bar ----
  ...
  const buttonW = WELCOME_BUTTONS.map((b) => b.length + 2 /* paddingX */ + 2 /* border */);
      ...
      <Box flexDirection="column" alignItems="center" flexShrink={0}>
        ...
        <Box flexDirection="column" borderStyle="round" borderColor={border} paddingX={2} width={cardW} flexShrink={0}>
          ...
          <Box flexDirection="row" paddingLeft={Math.max(0, Math.floor((cardContentW - buttonsRowW) / 2))}>
            {WELCOME_BUTTONS.map((label, i) => (
              <Box key={label} borderStyle="round" borderColor={i === 0 ? accent : border} paddingX={1} marginRight={i < WELCOME_BUTTONS.length - 1 ? BUTTON_GAP : 0}>
                <Text bold={i === 0} color={i === 0 ? accent : undefined} dimColor={i !== 0}>{label}</Text>
              </Box>
            ))}
          </Box>
          <Text> </Text>
        ...
        <Text> </Text>
        <Text color={muted}>Enter start · s setup · q quit</Text>
      </Box>
    </Box>
  );
}
